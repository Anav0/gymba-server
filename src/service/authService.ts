...
export interface IAuthService {
  ...
  reVerifyUser(email: string): Promise<void>;
...
export class AuthService implements IAuthService {
  ...
  async sendVerificationEmail(
    ...
    if (!user) throw new Error("No user with given email found");
    ...
    user.expireAt = moment(Date.now())
      .add(settings.user.validFor, settings.user.unit as moment.unitOfTime.Base)
      .valueOf();
